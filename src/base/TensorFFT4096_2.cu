...
__global__ void TensorFFT4096_2(__half* input_data_RE, __half* input_data_IM,
  ...
  if (inter_warp_id == 0) {
    for(int i=0; i<256;i++){
      output_data_RE[i + warp_global_memory_offset] = buffer_RE[i];
      output_data_IM[i + warp_global_memory_offset] = buffer_IM[i];
    }
  }

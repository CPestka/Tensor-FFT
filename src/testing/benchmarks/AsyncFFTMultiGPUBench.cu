...
int main(){
  ...
  for(int i=8; i<=log_length_max; i++){
    ...
    DataBatchHandlerMultiGPU my_handler(fft_length.back(),
                                        amount_of_asynch_ffts, device_list);
    ...
    for(int k=0; k<sample_size + warmup_samples; k++){
      ...
      error_mess = ComputeFFTsMultiGPU(my_plan, my_handler);
      if (error_mess) {
        std::cout << error_mess.value() << std::endl;
        return false;
      }
  ...
  WriteBenchResultsToFile(avg_runtime, sigma_runtime, fft_length,
                          (std::to_string(sample_size) + "_Multi_async_") +
